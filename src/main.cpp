...
constexpr double machRef = 0.07;
constexpr double plateLength = 0.001; // 0,01 mm
constexpr double aSeaLevel = 340.28;
constexpr double velRef = machRef * aSeaLevel;
constexpr double alfa = 0.0 * M_PI / 180.0;
const double velURef = velRef * cos(alfa);
const double velVRef = velRef * sin(alfa);
...
constexpr size_t NPOINTSX = 100;
constexpr size_t NPOINTSY = 100;
constexpr size_t NITERATIONS = 100000;
constexpr double LHORI = plateLength;
constexpr double ReL = RHOref * velRef*plateLength / muRef;
const double LVERT = 25.0 * plateLength / sqrt(ReL);
...
int main()
{
	...
	State defState = { velURef, velVRef, pressureSeaLevel, tempRef };
	...
	const double UINF = velURef;
	const double VINF = velVRef;

	// CASE 1 (LEADING EDGE)
	estado[0 + ((NPOINTSY - 1) * NPOINTSX)].P = PINF;
	estado[0 + ((NPOINTSY - 1) * NPOINTSX)].T = TINF;
	estado[0 + ((NPOINTSY - 1) * NPOINTSX)].u = 0.0;
	estado[0 + ((NPOINTSY - 1) * NPOINTSX)].v = 0.0;

	// CASE 2 (INFLOW/LEFT.TOP)

	for (size_t i = 0; i < NPOINTSY-1; i++)
	{
		estado[0 + (i*NPOINTSX)].P = PINF;
		estado[0 + (i*NPOINTSX)].T = TINF;
		estado[0 + (i*NPOINTSX)].u = UINF;
		estado[0 + (i*NPOINTSX)].v = VINF;
	}
	...
	for (size_t it = 0; it < NITERATIONS; it++)
	{
		...
		for (size_t i = 1; i < NPOINTSY-1; i++)
		{
			for (size_t j = 1; j < NPOINTSX-1; j++)
			{
				...
				const double vlij = (4.0 / 3.0) * gama * (mu / prandtlNumber) / CalcDensity(estadoij.P, R, estadoij.T);
		...
		if (it % 100 == 0)
		{
			std::string nend = std::to_string(it) + ".csv";
...
void CalcExtrapolationsBC(std::vector<State>& field, size_t NPOINTSX, size_t NPOINTSY)
{
	...
	//for (size_t j = 0; j < NPOINTSX; j++)
	//{
	//	const double vextr = (2.0 * field[j + ( 1 * NPOINTSX)].v) -
	//		(field[j + (2 * NPOINTSX)].v);
	//
	//	// v extrapolated from inside.
	//	field[j + (0 * NPOINTSX)].v = vextr;
	//}
}
